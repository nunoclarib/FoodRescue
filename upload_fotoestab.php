...
function crop_image($fileNameNew, $max_resolution)
{
    if ($fileNameNew) {
        ...
        if ($og_img) {
            ...
            $new_crop_image = imagecreatetruecolor(600, $max_resolution);

            imagecopyresampled($new_crop_image, $new_image, 0, 0, $x, $y, $max_resolution, 600, $max_resolution, 600);
    ...
    if (in_array($fileActualExt, $allowed)){
        ...
        if ($fileError===0){
            ...
            if ($fileSize < 13000000){
               ...
               header("Location: registo_estab_pic.php?msg=0");
            }
        }
        else{
            echo 'Erro ao fazer upload';
            header("Location: registo_estab_pic.php?msg=1");
        }
    }
    else{
        echo 'Não é suportado este tipo de ficheiro';
        header("Location: registo_estab_pic.php?msg=2");
    }

}
else{
    echo'Não existe file';
    header("Location: registo_estab_pic.php?msg=3");
}
